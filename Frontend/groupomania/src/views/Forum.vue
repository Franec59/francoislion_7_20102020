<template>
  <div class="container home pt-3">
    ...
    <div class="row">
      ...
      <div class="col-10 col-lg-6 mt-3 contact">
        <div v-for="data in messages" :key="data">
          <div class="card mt-5 post">
            <div class="card-body">
              <h3 class="card-title">{{ data.titre }}</h3>
              <p class="card-text post-text" v-if="data.contenu != null">{{ data.contenu }}</p>
              <p class="card-text post-text" v-else-if="off"></p>
        
            </div>
            <div class="post-image">
              <img :src="data.image" class="card-img-bottom rounded" alt="..." v-if="data.image != null">
              <img :src="data.image" class="card-img-bottom rounded" alt="..." v-else-if="imgoff">
            </div>
            <p class="card-text"><small class="text-muted">De {{ data.user_mess }} le {{ data.date_post }}</small></p>
            <div class="card-footer">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-trash ico-post" viewBox="0 0 16 16" v-on:click="delPost()">
...
.card-title{
  border-bottom: 1px solid rgba(48, 46, 46, 0.808);
  margin-left: 0.2rem;
  margin-right: 0.2rem;
  padding-bottom: 1rem;
}
